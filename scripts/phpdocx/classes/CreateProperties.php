            ...
            if (in_array($key, $generalProperties)) {
                ...
                if ($coreNodes->length > 0) {
                    ...
                    if ($key == 'created' || $key == 'modified') {
                        $strNode = '<' . $nameSpaces[$key] . ':' . $key . ' xmlns:' . $nameSpaces[$key] . '="' . $nameSpacesURI[$nameSpaces[$key]] . '" xsi:type="dcterms:W3CDTF">' . htmlspecialchars($value) . '</' . $nameSpaces[$key] . ':' . $key . '>';
                    } else {
                        $strNode = '<' . $nameSpaces[$key] . ':' . $key . ' xmlns:' . $nameSpaces[$key] . '="' . $nameSpacesURI[$nameSpaces[$key]] . '">' . htmlspecialchars($value) . '</' . $nameSpaces[$key] . ':' . $key . '>';
                    }
